...
function runTest() {
  mocha.describe('relativePositionsDefence()', function () {
    mocha.it('kickoff team defensive players move towards ball on opposite side', async () => {
      ...
      for (let player of matchDetails.kickOffTeam.players) {
        if (player.playerID == '78883930303030101') expect(player.relativePOS).to.eql([position[0], position[1]])
    ...
    mocha.it('kickoff team defensive players move towards ball on opposite side with player near', async () => {
      ...
      expect(matchDetails).to.be.an('object')
      for (let player of matchDetails.kickOffTeam.players) {
        let playerXProx = common.isBetween((position[0] - player.startPOS[0]), -40, 40)
        let playerYProx = common.isBetween((position[1] - player.startPOS[1]), -40, 40)
        if (player.playerID == '78883930303030101') expect(player.relativePOS).to.eql([position[0], position[1]])
        else if (playerXProx && playerYProx) expect(player.relativePOS).to.eql([position[0], position[1]])
        else expect(player.relativePOS).to.eql([player.originPOS[0], player.startPOS[1] - 20])
      }
    })
    mocha.it('secondteam defensive players move towards ball on opposite side', async () => {
      ...
      for (let player of matchDetails.secondTeam.players) {
        if (player.playerID == closestPlayer.playerID) expect(player.relativePOS).to.eql(ballPosition)
        else expect(player.relativePOS).to.eql([player.originPOS[0], player.startPOS[1] - 20])
      }
    })
    mocha.it('kickoff team defensive players ball in own half', async () => {
      ...
      for (let playerNum of [0, 1, 2, 3, 4]) {
        ...
        if (thisPlayer.playerID == closestPlayer.playerID) expect(thisPlayer.relativePOS).to.eql(ballPosition)
        else expect(thisPlayer.relativePOS).to.eql(thisPlayer.originPOS)
      }
  ...
  mocha.describe('relativePositionsLooseBall()', function () {
    mocha.it('kickoff team moves towards ball', async () => {
      ...
      expect(kickOffTeam.players[1].relativePOS).to.eql([matchDetails.ball.position[0], matchDetails.ball.position[1]])
      expect(kickOffTeam.players[2].relativePOS).to.eql([230, 101])
    ...
    mocha.it('second team moves towards ball', async () => {
      ...
      expect(secondTeam.players[2].relativePOS).to.eql([230, 950])
