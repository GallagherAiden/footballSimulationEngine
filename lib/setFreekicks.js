...
function setTopOneHundredToHalfwayYPos(matchDetails, attack, defence) {
  ...
  for (let player of attack.players) {
    if (kickPlayer.position == 'GK') {
      ...
      if (player.playerID != kickPlayer.playerID) player.startPOS = [player.originPOS[0], player.originPOS[1] + 300]
    } else {
      let newYPOS = player.originPOS[1] + (ball.position[1] - player.originPOS[1]) + 300
      if (player.playerID == kickPlayer.playerID) player.startPOS = ball.position.map(x => x)
  ...
  for (let player of defence.players) {
    if (kickPlayer.position == 'GK') {
      if (player.position == 'GK') player.startPOS = player.originPOS.map(x => x)
      if (player.position != 'GK') player.startPOS = [player.originPOS[0], player.originPOS[1] - 100]
    } else if (['GK', 'CB', 'LB', 'RB'].includes(player.position)) player.startPOS = player.originPOS.map(x => x)
    else if (['CM', 'LM', 'RM'].includes(player.position)) {
...
function setBottomOneHundredToHalfwayYPos(matchDetails, attack, defence) {
  ...
  for (let player of defence.players) {
    if (kickPlayer.position == 'GK') {
      ...
      player.startPOS = [player.originPOS[0], parseInt((pitchHeight * 0.25) - 5, 10)]
    } else {
      player.startPOS = [player.originPOS[0], parseInt(pitchHeight * 0.5, 10)]
    }
  }
...
function setTopHalfwayToBottomQtrYPos(matchDetails, attack, defence) {
  ...
  for (let player of attack.players) {
    if (player.position == 'GK') player.startPOS = [player.originPOS[0], parseInt(pitchHeight * 0.25, 10)]
      ...
      if (player.playerID != kickPlayer.playerID) player.startPOS = [player.originPOS[0], parseInt(maxYPOSCheck, 10)]
    } else {
...
function setBottomHalfwayToTopQtrYPos(matchDetails, attack, defence) {
  ...
  kickPlayer.startPOS = ball.position.map(x => x)
  for (let player of attack.players) {
    if (player.position == 'GK') player.startPOS = [player.originPOS[0], parseInt(pitchHeight * 0.75, 10)]
    else if (['CB', 'LB', 'RB'].includes(player.position)) {
  ...
  for (let player of defence.players) {
    if (['GK', 'CB', 'LB', 'RB'].includes(player.position)) {
      ...
      player.startPOS = [player.originPOS[0], parseInt(pitchHeight * 0.25, 10)]
    } else {
      player.startPOS = [player.originPOS[0], parseInt(pitchHeight * 0.5, 10)]
    }
  }
  ...
  for (let player of attack.players) {
    if (player.position == 'GK') player.startPOS = [player.originPOS[0], parseInt(pitchHeight * 0.25, 10)]
      ...
      if (player.position == 'CB') {
        ...
        player.startPOS = [player.originPOS[0], parseInt(pitchHeight * 0.66, 10)]
      }
    } else if (player.playerID != kickPlayer.playerID) {
      player.startPOS[0] = common.getRandomNumber(boundaryX[0], boundaryX[1])
      player.startPOS[1] = common.getRandomNumber(boundaryY[0], boundaryY[1])
    }
  }
  let playerSpace = -3
  for (let player of defence.players) {
    ...
    if (player.position == 'GK') player.startPOS = player.startPOS = player.originPOS.map(x => x)
    else if (['CB', 'LB', 'RB'].includes(player.position)) {
      player.startPOS = [midWayFromBalltoGoalX + playerSpace, midWayFromBalltoGoalY]
      playerSpace += 2
    } else {
      player.startPOS[0] = common.getRandomNumber(boundaryX[0], boundaryX[1])
      player.startPOS[1] = common.getRandomNumber(boundaryY[0], boundaryY[1])
  ...
  for (let player of attack.players) {
    if (player.position == 'GK') player.startPOS = [player.originPOS[0], parseInt(pitchHeight * 0.75, 10)]
      ...
      if (player.position == 'CB') {
        ...
        player.startPOS = [player.originPOS[0], parseInt(pitchHeight * 0.33, 10)]
      }
    } else if (player.playerID != kickPlayer.playerID) {
      player.startPOS[0] = common.getRandomNumber(boundaryX[0], boundaryX[1])
      player.startPOS[1] = common.getRandomNumber(boundaryY[0], boundaryY[1])
...
function setTopLowerFinalQtrBylinePos(matchDetails, attack, defence) {
  ...
  for (let player of defence.players) {
    ...
    if (player.position == 'GK') player.startPOS = player.startPOS = player.originPOS.map(x => x)
      ...
      player.startPOS = [midWayFromBalltoGoalX, playerSpace]
      playerSpace -= 2
    } else {
      player.startPOS[0] = common.getRandomNumber(boundaryX[0], boundaryX[1])
      player.startPOS[1] = common.getRandomNumber(boundaryY[0], boundaryY[1])
