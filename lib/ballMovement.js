...
function moveBall(matchDetails) {
  return new Promise(function(resolve, reject) {
    ...
    resolveBallMovement(ballPlayer, ball.position, ballPosition, power, kickOffTeam, secondTeam, matchDetails).then(function(endPosition) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    async.eachSeries(teammates.players, function eachPlayer(teamPlayer, teamPlayerCallback) {
      ...
      if (player.skill.passing > common.getRandomNumber(0, 100)) {
        if (player.originPOS[1] > pitchHeight / 2) {
          closestPlayerPosition[0] = targetPlayerXPos
          closestPlayerPosition[1] = targetPlayerYPos - 10
        } else {
          closestPlayerPosition[0] = targetPlayerXPos
          closestPlayerPosition[1] = targetPlayerYPos + 10
        }
...
async function resolveBallMovement(player, currentPOS, newPOS, power, team, opposition, matchDetails) {
  ...
  for (const thisPos of lineToEndPosition) {
    let playerInformation = await setPositions.closestPlayerToPosition(player, team, thisPos).catch(function(err) {
      const error = `Error when getting this teams closest player to that position: ${err}`
      throw error
    })
    ...
    if (thisTeamPlayer && thisTeamPlayer.startPOS[0] === thisPos[0] && thisTeamPlayer.startPOS[1] === thisPos[1]) {
      ...
      let playerInformation = setPositions.closestPlayerToPosition(player, opposition, thisPos).catch(function(err) {
        const error = `Error when getting this teams closest player to that position: ${err}`
        throw error
      })
  ...
  if (!deflectionPlayer) {
    let finalPosition = await setPositions.keepInBoundaries(newPOS, player.originPOS[1], matchDetails).catch(function(err) {
      const error = `Error when keeping ball in boundaries: ${err}`
      throw error
    })
...
function resolveDeflection(power, currentPOS, deflectionPosition, deflectionPlayer, deflectionTeam, matchDetails) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (newPower < 75) {
      ...
      return resolve(deflectionPosition)
    }
...
function mergeArrays(arrayLength, oldPos, newPos, array1, array2, array3) {
  return new Promise(function(resolve) {
    ...
    let newArray = []
    async.eachSeries(
      arrayN, function eachN(thisn, nCallback) {
