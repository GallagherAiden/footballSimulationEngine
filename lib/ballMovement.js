...
async function moveBall(matchDetails) {
  ...
  let endPos = await resolveBallMovement(bPlayer, bPosition, ballPos, power, kickOffTeam, secondTeam, matchDetails)
    .catch(function(err) {
...
async function ballKicked(matchDetails, player) {
  ...
  let BOIts = await mergeArrays(powerArray.length, position, newPosition, xArray, yArray, powerArray)
    .catch(function(err) {
      const error = `Unable to merge arrays: ${err}`
      throw error
    })
  matchDetails.ball.ballOverIterations = BOIts
  let endPos = await resolveBallMovement(player, position, BOIts[0], power, kickOffTeam, secondTeam, matchDetails)
    .catch(function(err) {
      const error = `Error when resolving the ball movement: ${err}`
      throw error
    })
...
async function shotMade(matchDetails, team, opp, player) {
  ...
  if (player.originPOS[1] > pitchHeight / 2) {
    ...
    if (common.isBetween(shotPower, distanceFromGoal, distanceFromGoal + 50)) {
      if (player.skill.shooting > common.getRandomNumber(0, 20)) {
        ...
        shotPosition[1] = 0
        let endPos = await resolveBallMovement(player, PlyPos, shotPosition, shotPower, team, opp, matchDetails)
          .catch(function(err) {
        ...
        if (oppXpos && oppYpos) {
          ...
          await setPositions.setGoalScored(team, opp, matchDetails)
            .catch(function(err) {
      ...
      let endPos = await setPositions.setGoalKick(opp, team, matchDetails)
        .catch(function(err) {
...
async function throughBall(matchDetails, teammates, player) {
  ...
  let BOIts = await mergeArrays(powerArray.length, position, closestPlayerPosition, xArray, yArray, powerArray)
    .catch(function(err) {
      const error = `Unable to merge arrays: ${err}`
      throw error
    })
...
async function resolveBallMovement(player, currentPOS, newPOS, power, team, opp, matchDetails) {
  let defPlayer
  let defPosition
  let defTeam
  ...
  for (const thisPos of lineToEndPosition) {
    ...
    if (thisTeamPlayer) {
      ...
      try {
        ...
        const error = `Error getting closest ${opp.name} player: ${err}`
        throw error
      }
  ...
  let newPos = await resolveDeflection(power, currentPOS, defPosition, defPlayer, defTeam, matchDetails)
    .catch(function(err) {
...
async function resolveDeflection(power, currentPOS, defPosition, defPlayer, defTeam, matchDetails) {
  let { kickOffTeam, secondTeam } = matchDetails
