...
function ballKicked(matchDetails, player) {
  ...
  if (player.originPOS[1] > (pitchHeight / 2)) {
    ...
    if (direction === `wait`) newPos = newKickedPosition(position, 0, (power / 2), 0, (power / 2))
    else if (direction === `north`) newPos = newKickedPosition(position, -20, 20, -power, -(power / 2))
    else if (direction === `east`) newPos = newKickedPosition(position, (power / 2), power, -20, 20)
    else if (direction === `west`) newPos = newKickedPosition(position, -power, -(power / 2), -20, 20)
    else if (direction === `northeast`) newPos = newKickedPosition(position, 0, (power / 2), -power, -(power / 2))
    else if (direction === `northwest`) newPos = newKickedPosition(position, -(power / 2), 0, -power, -(power / 2))
  } else {
    direction = teamShootingToBottom[common.getRandomNumber(0, teamShootingToBottom.length - 1)]
    if (direction === `wait`) newPos = newKickedPosition(position, 0, (power / 2), 0, (power / 2))
    else if (direction === `east`) newPos = newKickedPosition(position, (power / 2), power, -20, 20)
    else if (direction === `west`) newPos = newKickedPosition(position, -120, 0, -30, 30)
    else if (direction === `south`) newPos = newKickedPosition(position, -20, 20, (power / 2), power)
    else if (direction === `southeast`) newPos = newKickedPosition(position, 0, (power / 2), (power / 2), power)
    else if (direction === `southwest`) newPos = newKickedPosition(position, -(power / 2), 0, (power / 2), power)
  }
...
function penaltyTaken(matchDetails, team, opp, player) {
  const [pitchWidth, pitchHeight] = matchDetails.pitchSize
  player.action = `none`
...
function getPlayersInDistance(teammates, player, pitchSize) {
...
function resolveDeflection(power, thisPOS, defPosition, defPlayer, defTeam, matchDetails) {
  ...
  if (newPower < 75) {
    setLowNewPower(matchDetails, kickOffTeam, defPlayer, defTeam)
    return defPosition
  }
...
function setDeflectionPlayerOffside(matchDetails, defTeam, defPlayer) {
  ...
  if (defTeam.name == kickOffTeam.name) matchDetails = setPositions.setSetpieceKickOffTeam(matchDetails)
  else matchDetails = setPositions.setSetpieceSecondTeam(matchDetails)
...
function setLowNewPower(matchDetails, kickOffTeam, defPlayer, defTeam) {
  defPlayer.hasBall = true
  matchDetails.ball.lastTouch = defPlayer.name
  if (defPlayer.offside == true) {
    matchDetails.iterationLog.push(`${defPlayer.name} is offside. Set piece given`)
...
function calcBallMovementOverTime(matchDetails, stength, nextPosition, player) {
  let { kickOffTeam, secondTeam } = matchDetails
  let { position } = matchDetails.ball
  const power = common.calculatePower(stength)
