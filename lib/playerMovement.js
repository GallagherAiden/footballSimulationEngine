...
function team1atBottom(team1, team2, pitchHeight) {
  ...
  let btmPlayerOffsidePosition = common.isBetween(btmPlayer.currentPOS[1], offsideT2Yposition.pos2, offsideT2Yposition.pos1) 
  if(btmPlayerOffsidePosition && btmPlayer.hasBall) return 
  for (let thisPlayer of team2.players) {
    thisPlayer.offside = false
    if (common.isBetween(thisPlayer.currentPOS[1], offsideT2Yposition.pos2, offsideT2Yposition.pos1)) {
      if (!thisPlayer.hasBall) thisPlayer.offside = true
    }
  }
}
...
function team1atTop(team1, team2, pitchHeight) {
  ...
  let topPlayerOffsidePosition = common.isBetween(topPlayer.currentPOS[1], offsideT2Yposition.pos1, offsideT2Yposition.pos2) 
  if(topPlayerOffsidePosition && topPlayer.hasBall) return 
  for (let thisPlayer of team2.players) {
    thisPlayer.offside = false
