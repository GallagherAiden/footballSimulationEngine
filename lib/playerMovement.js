...
function decideMovement(closestPlayer, team, opposition, matchDetails) {
  ...
  for (const thisPlayer of team.players) {
    ...
    if (thisPlayer.hasBall == true) { console.log(`Ball Position: ${ballToPlayerX}, ${ballToPlayerY}`) }
    let possibleActions
    ...
    if (xPosition && yPosition && withTeam !== team.name) {
      if (thisPlayer.startPOS[0] === position[0] && thisPlayer.startPOS[1] === position[1]) {
        if (withPlayer === true && thisPlayer.hasBall === false && withTeam !== team.name) {
          if (action === `tackle`) {
            ...
            if (foul) {
              ...
              setPositions.setSetpiece(matchDetails, opposition, team)
            }
          }
        } else {
        ...
        thisPlayer.hasBall = true
        matchDetails.ball.ballOverIterations = []
        matchDetails.ball.position = thisPlayer.startPOS.map(x => x)
...
function makeMovement(player, action, opposition, ballX, ballY, matchDetails) {
  ...
  if (action === `wait`) {
    ...
    let formationDirection = setPositions.formationCheck(player.relativePOS, player.startPOS)
...
function closestPlayerToBall(closestPlayer, team, matchDetails) {
  try {
    ...
    setPositions.setRelativePosition(closestPlayerDetails, team, matchDetails)
