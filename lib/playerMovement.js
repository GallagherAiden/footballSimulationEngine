...
function decideMovement(closestPlayer, team, opp, matchDetails) {
  ...
  for (const thisPlayer of team.players) {
    if (thisPlayer.currentPOS[0] != 'NP') {
      ...
      checkProvidedAction(matchDetails, thisPlayer, action)
      if (withTeam && withTeam !== team.teamID && closestPlayer.name === thisPlayer.name) {
        closestPlayerAction(action, ballToPlayerX, ballToPlayerY)
      }
...
function closestPlayerAction(action, ballToPlayerX, ballToPlayerY) {
  if (action !== `tackle` && action !== `slide` && action !== `intercept`) action = `sprint`
  if (common.isBetween(ballToPlayerX, -30, 30) === false) {
    if (ballToPlayerX > 29) ballToPlayerX = 29
    else ballToPlayerX = -29
  }
  if (common.isBetween(ballToPlayerY, -30, 30) === false) {
    if (ballToPlayerY > 29) ballToPlayerY = 29
    else ballToPlayerY = -29
  }
}
...
function getMovement(player, action, opposition, ballX, ballY, matchDetails) {
  ...
  if (action === `wait` || ballActions.includes(action)) return [0, 0]
    ...
    return getRunMovement(player, matchDetails, ballX, ballY)
  } else if (action === `sprint`) {
    return getSprintMovement(player, matchDetails, ballX, ballY)
  }
}
...
function getInterceptTrajectory(opposition, ballPosition, pitchSize) {
  ...
  for (let i of new Array(highNum)) {
...
function getRunMovement(player, matchDetails, ballX, ballY) {
  let move = [0, 0]
  ...
  if (common.isBetween(ballX, -30, 30) && common.isBetween(ballY, -30, 30)) {
    if (common.isBetween(ballX, -30, 1)) move[0] = movementRun[common.getRandomNumber(1, 2)]
    else if (common.isBetween(ballX, -1, 30)) move[0] = movementRun[common.getRandomNumber(0, 1)]
    else move[0] = movementRun[common.getRandomNumber(1, 1)]
    if (common.isBetween(ballY, -30, 1)) move[1] = movementRun[common.getRandomNumber(1, 2)]
    else if (common.isBetween(ballY, -1, 30)) move[1] = movementRun[common.getRandomNumber(0, 1)]
...
function getSprintMovement(player, matchDetails, ballX, ballY) {
  let move = [0, 0]
  ...
  if (common.isBetween(ballX, -30, 30) && common.isBetween(ballY, -30, 30)) {
    if (common.isBetween(ballX, -30, 1)) move[0] = movementSprint[common.getRandomNumber(2, 4)]
    else if (common.isBetween(ballX, -1, 30)) move[0] = movementSprint[common.getRandomNumber(0, 2)]
    else move[0] = movementSprint[common.getRandomNumber(2, 2)]
    if (common.isBetween(ballY, -30, 1)) move[1] = movementSprint[common.getRandomNumber(2, 4)]
    else if (common.isBetween(ballY, -1, 30)) move[1] = movementSprint[common.getRandomNumber(0, 2)]
