...
function resolveTackle(player, team, opposition, matchDetails) {
  ...
  if (calcTackleScore(player.skill, 5) > calcRetentionScore(thatPlayer.skill, 5)) {
    //TODO DIFFERENTIATE AGAINST SLIDE AND TACKLE!!!! e.g. setPostTacklePosition (1 or 3), setInjury....
    setSuccessTackle(matchDetails, team, opposition, player, thatPlayer)
    return false
  } else {
    setFailedTackle(matchDetails, player, thatPlayer)
    return false
  }
}
...
function setFailedTackle(matchDetails, player, thatPlayer) {
  matchDetails.iterationLog.push(`Failed tackle by: ${player.name}`)
  player.stats.tackles.off++
  setInjury(matchDetails, player, thatPlayer, 15000, 14000)
  setPostTacklePosition(matchDetails, thatPlayer, player, 3)
}

function setSuccessTackle(matchDetails, team, opposition, player, thatPlayer) {
  ...
  setInjury(matchDetails, thatPlayer, player, 14000, 15000)
  setPostTacklePosition(matchDetails, player, thatPlayer, 3)
}
