...
function findPossActions(player, team, opposition, ballX, ballY, matchDetails) {
  ...
  if (hasBall === false) {
    ...
    parameters = playerDoesNotHaveBall(player, ballX, ballY, matchDetails)
  } else if (originPOS[1] > (pitchHeight / 2)) {
    bottomTeamPlayerHasBall(matchDetails, player, team, opposition)
  } else if (position === 'GK') {
    //bottom team goalie
  ...
  } else if (checkPositionInBottomPenaltyBox(position, pitchWidth, pitchHeight)) {
    //top team player has ball
    //[0]shoot, [1]throughBall, [2]pass, [3]cross, [4]tackle, [5]intercept
    //[6]slide, [7]run, [8]sprint //[9]cleared //[10]boot
    if (checkPositionInBottomPenaltyBoxClose(position, pitchWidth, pitchHeight)) {
...
function bottomTeamPlayerHasBall(matchDetails, player, team, opposition) {
  let playerInformation = setPositions.closestPlayerToPosition(player, opposition, player.currentPOS)
  let playerProx = [Math.abs(playerInformation.proxPOS[0]), Math.abs(playerInformation.proxPOS[1])]
  const [pitchWidth, pitchHeight] = matchDetails.pitchSize
  let {
    position, currentPOS, skill
  } = player
  if (position === 'GK' && playerProx[0] < 10 && playerProx[1] < 25) return [0, 0, 10, 0, 0, 0, 0, 10, 0, 40, 40]
    ...
    bottomTeamPlayerHasBallInTopPenaltyBox(matchDetails, player, team, opposition)
    ...
    if (playerProx[0] < 10 && playerProx[1] < 10) {
  ...
  } else if (playerProx[0] < 10 && playerProx[1] < 10) {
...
function bottomTeamPlayerHasBallInTopPenaltyBox(matchDetails, player, team, opposition) {
  ...
  if (checkPositionInTopPenaltyBoxClose(currentPOS, pitchWidth, pitchHeight)) {
    if (oppositionNearPlayer(playerInformation, 20)) {
...
function oppositionNearPlayer(oppositionPlayer, space) {
  let playerProximity = [Math.abs(oppositionPlayer.proxPOS[0]), Math.abs(oppositionPlayer.proxPOS[1])]
  if (playerProximity[0] < space && playerProximity[1] < space) return true
  return false
}
