...
function setPlayerPositions(matchDetails, team, extra) {
  for (const thisPlayer of team.players) {
    if (thisPlayer.position == `GK`) {
      ...
      thisPlayer.intentPOS = tempArray.map(x => x)
      thisPlayer.intentPOS[1] = playerPos
    }
  }
}
...
function setintentPOSition(matchDetails, closestPlayer) {
  ...
  if (defendingTeam != 'none') setDefenceintentPOS(matchDetails, defendingTeam, closestPlayer)
  if (kickTeam != 'none') setAttackintentPOS(matchDetails, kickTeam)
...
function setLooseintentPOS(matchDetails, thisTeam, closestPlayer) {
  ...
  for (let player of thisTeam.players) {
    ...
    if (player.name == closestPlayer.name) player.intentPOS = ball.position.map(x => x)
    else if (common.isBetween(diffXPOSplayerandball, -16, 16) && common.isBetween(diffYPOSplayerandball, -16, 16)) {
      player.intentPOS = ball.position.map(x => x)
    } else {
...
function setDefenceintentPOS(matchDetails, defendingTeam, closestPlayer) {
  const [, pitchHeight] = matchDetails.pitchSize
  let { ball } = matchDetails
...
function setAttackintentPOS(matchDetails, kickingTeam) {
  const [, pitchHeight] = matchDetails.pitchSize
