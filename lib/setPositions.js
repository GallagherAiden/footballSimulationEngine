...
function setCornerPositions(team, opposition, side, matchDetails) {
  ...
  return new Promise(function(resolve) {
    ...
    return resolve(matchDetails.ball.position)
  })
}
...
function closestPlayerToPosition(player, team, position) {
  return new Promise(function(resolve) {
    ...
    async.eachSeries(team.players, function eachPlayer(thisPlayer, thisPlayerCallback) {
      ...
      return resolve(playerInformation)
    })
  })
}
...
function setSetpiece(matchDetails, team, opposition) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (team.players[0].originPOS[1] > matchHeight / 2) {
      if (common.isBetween(ballPosition[0], (matchWidth / 4) - 5, matchWidth - (matchWidth / 4) + 5) && common.isBetween(ballPosition[1], 0, (matchHeight / 6) - 5)) {
        setPenalty(team, opposition, 'top', matchDetails).then(function() {
          ...
          const error = `Error whilst setting the penalty for ${team.name}: ${err}`
          reject(error)
        })
      } else {
        setFreekick(ballPosition, team, opposition, 'top', matchDetails).then(function() {
          ...
          const error = `Error whilst setting the freekick for ${team.name}: ${err}`
          reject(error)
        })
      }
    } else if (common.isBetween(ballPosition[0], (matchWidth / 4) - 5, matchWidth - (matchWidth / 4) + 5) && common.isBetween(ballPosition[1], matchHeight - (matchHeight / 6) + 5, matchHeight)) {
      ...
      if (ballIntended[1] < 0) {
        ...
        if (kickersSide > matchHeight / 2) {
          ...
          setCornerPositions(secondTeam, kickOffTeam, side, matchDetails).then(function(ballIntended) {
            ...
            const error = `Error when setting corner: ${err}`
            reject(error)
          })
        }
      ...
      closestPlayerToPosition('none', kickOffTeam, ballIntended).then(function(playerInformationA) {
        closestPlayerToPosition('none', secondTeam, ballIntended).then(function(playerInformationB) {
          ...
          if (teamAPlayer && teamAPlayer[0] === ballIntended[0] && teamAPlayer[1] === ballIntended[1]) {
            ...
            ballIntended = [matchWidth / 2, matchHeight / 2]
            ...
            if (matchDetails.half === 1) {
              setGoalScored(secondTeam, kickOffTeam, matchDetails).then(function() {
                ...
                const error = `Error when processing the goal: ${err}`
                reject(error)
                reject(matchDetails.iterationLog)
              })
            } else {
              setGoalScored(kickOffTeam, secondTeam, matchDetails).then(function() {
                matchDetails.kickOffTeamStatistics.goals++
          ...
          const error = `Error when finding the closest player to ball for accidental goal ${err}`
          reject(error)
        })
      }).catch(function(err) {
        const error = `Error when finding the closest player to ball for accidental goal${err}`
...
function formationCheck(origin, current) {
  return new Promise(function(resolve, reject) {
    try {
      ...
      return resolve(moveToFormation)
    } catch (error) {
      reject(error)
    }
  })
