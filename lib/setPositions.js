...
function setTopRightCornerPositions(matchDetails) {
  ...
  setBallSpecificCornerValue(matchDetails)
  return matchDetails
}
...
function setBallSpecificCornerValue(matchDetails) {
  let kickOffTeamKeepYPos = matchDetails.kickOffTeam.players[0].originPOS[1]
  let halfPitchSize = matchDetails.pitchSize[1] / 2
  let attack = (kickOffTeamKeepYPos > halfPitchSize) ? matchDetails.kickOffTeam : matchDetails.secondTeam
...
function setSetpieceKickOffTeam(matchDetails) {
  ...
  let attackingTowardsTop = (matchDetails.kickOffTeam.players[0].startPOS[1] > matchHeight / 2) ? true : false
  if (attackingTowardsTop && ballInPenalyBoxX && ballInTopPenalyBoxY) {
    setTopPenalty(matchDetails)
    matchDetails.kickOffTeamStatistics.penalties++
  } else if (attackingTowardsTop == false && ballInPenalyBoxX && ballInBottomPenalyBoxY) {
    setBottomPenalty(matchDetails)
    matchDetails.kickOffTeamStatistics.penalties++
  } else {
    setFreekick(ballPosition, matchDetails.kickOffTeam, matchDetails.secondTeam, `top`, matchDetails)
...
function setSetpieceSecondTeam(matchDetails) {
  ...
  let ballInBottomPenalyBoxY = common.isBetween(ballPosition[1], matchHeight - (matchHeight / 6) + 5, matchHeight)
  let ballInTopPenalyBoxY = common.isBetween(ballPosition[1], 0, (matchHeight / 6) - 5)
  let attackingTowardsTop = (matchDetails.secondTeam.players[0].startPOS[1] > matchHeight / 2) ? true : false
  if (attackingTowardsTop && ballInPenalyBoxX && ballInTopPenalyBoxY) {
    setTopPenalty(matchDetails)
    ...
    setFreekick(ballPosition, matchDetails.secondTeam, matchDetails.secondTeam, `top`, matchDetails)
  ...
  if (ballIntended[0] < 0 || ballIntended[0] > matchWidth || ballIntended[1] < 0 || ballIntended[1] > matchHeight) {
    ...
    if (ballIntended[1] < 0) {
      ...
      if (ballIntended[0] > matchWidth / 2) {
        let newBallIntended = setTopRightCornerPositions(matchDetails)
        return newBallIntended
      } else {
        let newBallIntended = setTopLeftCornerPositions(matchDetails)
        return newBallIntended
      }
      ...
      if (kickersSide > matchHeight / 2) {
        ...
        if (ballIntended[0] > matchWidth / 2) {
          let newBallIntended = setBottomRightCornerPositions(matchDetails)
          return newBallIntended
        } else {
          let newBallIntended = setBottomLeftCornerPositions(matchDetails)
          return newBallIntended
        }
      }
      setBottomGoalKick(secondTeam, kickOffTeam, matchDetails)
      return newBallIntended
    }
...
function removeBallFromAllPlayers(matchDetails) {
  for (player of matchDetails.kickOffTeam.players) {
    player.hasBall = false
  }
  for (player of matchDetails.secondTeam.players) {
    player.hasBall = false
  }
}
