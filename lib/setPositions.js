...
function setPlayerPositions(matchDetails, team, extra) {
  for (const thisPlayer of team.players) {
    if (thisPlayer.position == `GK`) thisPlayer.startPOS = thisPlayer.originPOS.map(x => x)
    else {
      thisPlayer.startPOS = thisPlayer.originPOS.map(x => x)
      const playerPos = parseInt(thisPlayer.startPOS[1], 10) + extra
      if (common.isBetween(playerPos, -1, (matchDetails.pitchSize[1] + 1))) thisPlayer.startPOS[1] = playerPos
...
function setRelativePosition(matchDetails, closestPlayer) {
  ...
  if (kickTeam == 'none' && defendingTeam == 'none') {
    setLooseRelativePos(matchDetails, kickOffTeam, closestPlayer)
    setLooseRelativePos(matchDetails, secondTeam, closestPlayer)
  }
}

function setLooseRelativePos(matchDetails, thisTeam, closestPlayer) {
  ...
  for (let player of thisTeam.players) {
    ...
    if (player.playerID == closestPlayer.playerID) player.relativePOS = [ball.position[0], ball.position[1]]
    else if (common.isBetween(diffXPOSplayerandball, -16, 16) && common.isBetween(diffYPOSplayerandball, -16, 16)) {
      player.relativePOS = [ball.position[0], ball.position[1]]
    } else {
      ...
      let newYPOS
      if (side == 'top' && northwards) player.relativePOS = player.originPOS.map(x => x)
      ...
      player.relativePOS = [player.originPOS[0], newYPOS]
    }
  }
}
...
function setNewRelativeTopYPOS(pitchHeight, player, diff) {
  let { position } = player
  if (position == 'GK') return common.upToMax(player.startPOS[1] + diff, pitchHeight * 0.15)
  if (position == 'CB') return common.upToMax(player.startPOS[1] + diff, pitchHeight * 0.25)
...
function setNewRelativeBottomYPOS(pitchHeight, player, diff) {
  let { position } = player
  if (position == 'GK') return common.upToMin(player.startPOS[1] + diff, pitchHeight * 0.85)
  if (position == 'CB') return common.upToMin(player.startPOS[1] + diff, pitchHeight * 0.75)
