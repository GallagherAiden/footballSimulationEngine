...
function setLooseRelativePos(matchDetails, thisTeam, closestPlayer) {
  ...
  for (let player of thisTeam.players) {
    ...
    if (player.name == closestPlayer.name) player.relativePOS = ball.position.map(x => x)
      ...
      if (side == 'top' && northwards) player.relativePOS = player.originPOS.map(x => x)
        ...
        if (common.isBetween(diffYPOSplayerandball, -100, 100)) {
          newYPOS = player.originPOS.map(x => x)
...
function setDefenceRelativePos(matchDetails, defendingTeam, closestPlayer) {
  ...
  for (let player of defendingTeam.players) {
    ...
    if (common.isBetween(diffXPOSplayerandball, -40, 40) && common.isBetween(diffYPOSplayerandball, -40, 40)) {
      ...
      if (player.name == closestPlayer.name) { 
        player.relativePOS = ball.position.map(x => x)
        player.relativePOS.pop()
      }
