...
async function setThrowIn(team, opposition, side, place, matchDetails) {
  const [matchWidth] = matchDetails.pitchSize
  let movement = team.players[5].originPOS[1] - place
  let oppMovement = 0 - movement
  if (side === 'left') {
    await setPlayerPositions(matchDetails, team, movement)
      .catch(function(err) {
        const error = `Error when setting throwin player positions: ${err}`
        throw error
      })
    ...
    matchDetails.ball.withTeam = team.name
    await setPlayerPositions(matchDetails, opposition, oppMovement)
      .catch(function(err) {
        const error = `Error when setting opposition throwin positions: ${err}`
        throw error
      })
  ...
  setPlayerPositions(matchDetails, team, movement)
    .catch(function(err) {
      const error = `Error when setting thowin player positions: ${err}`
      throw error
    })
...
async function setGoalKick(team, opposition, matchDetails) {
  const [matchWidth, matchHeight] = matchDetails.pitchSize
  if (team.players[0].originPOS[1] > matchHeight / 2) {
    await setPlayerPositions(matchDetails, team, -80)
      .catch(function(err) {
        const error = `Error when resetting player positions: ${err}`
        throw error
      })
    await setVariables.resetPlayerPositions(opposition)
      .catch(function(err) {
        const error = `Error when setting player positions: ${err}`
        throw error
      })
...
async function setSetpiece(matchDetails, team, opposition) {
  const [matchWidth, matchHeight] = matchDetails.pitchSize
...
function setPenalty(team, opposition, side, matchDetails) {
  const [matchWidth, matchHeight] = matchDetails.pitchSize
  let tempArray = (side === 'top') ? [matchWidth / 2, matchHeight / 6] : [matchWidth / 2, matchHeight - (matchHeight / 6)]
  let shootArray = (side === 'top') ? [matchWidth / 2, 60] : [matchWidth / 2, matchHeight - 60]
...
function setFreekick(ballPosition, team, opposition, side, matchDetails) {
  ...
  if (side === 'top') {
    ...
    if (ballPosition[1] > (matchHeight - (matchHeight / 3))) {
      ...
      team.players[1].startPOS = [team.players[1].originPOS[0], (matchHeight - (matchHeight / 6) - (600 - team.players[0].startPOS[1]) - (600 - team.players[1].originPOS[1]))]
      team.players[2].startPOS = [team.players[2].originPOS[0], (matchHeight - (matchHeight / 6) - (600 - team.players[0].startPOS[1]) - (600 - team.players[2].originPOS[1]))]
      team.players[3].startPOS = [team.players[3].originPOS[0], (matchHeight - (matchHeight / 6) - (600 - team.players[0].startPOS[1]) - (600 - team.players[3].originPOS[1]))]
      team.players[4].startPOS = [team.players[4].originPOS[0], (matchHeight - (matchHeight / 6) - (600 - team.players[0].startPOS[1]) - (600 - team.players[4].originPOS[1]))]
      team.players[5].startPOS = [team.players[5].originPOS[0], (matchHeight - (matchHeight / 6) - (600 - team.players[0].startPOS[1]) - (600 - team.players[5].originPOS[1]))]
      team.players[6].startPOS = [team.players[6].originPOS[0], (matchHeight - (matchHeight / 6) - (600 - team.players[0].startPOS[1]) - (600 - team.players[6].originPOS[1]))]
      team.players[7].startPOS = [team.players[7].originPOS[0], (matchHeight - (matchHeight / 6) - (600 - team.players[0].startPOS[1]) - (600 - team.players[7].originPOS[1]))]
      team.players[8].startPOS = [team.players[8].originPOS[0], (matchHeight - (matchHeight / 6) - (600 - team.players[0].startPOS[1]) - (600 - team.players[8].originPOS[1]))]
      team.players[9].startPOS = [team.players[9].originPOS[0], (matchHeight - (matchHeight / 6) - (600 - team.players[0].startPOS[1]) - (600 - team.players[9].originPOS[1]))]
      team.players[10].startPOS = [team.players[10].originPOS[0], (matchHeight - (matchHeight / 6) - (600 - team.players[0].startPOS[1]) - (600 - team.players[10].originPOS[1]))]
      opposition.players[1].startPOS = [opposition.players[1].originPOS[0], opposition.players[1].originPOS[1] + (matchHeight / 6)]
      opposition.players[2].startPOS = [opposition.players[2].originPOS[0], opposition.players[2].originPOS[1] + (matchHeight / 6)]
      opposition.players[3].startPOS = [opposition.players[3].originPOS[0], opposition.players[3].originPOS[1] + (matchHeight / 6)]
      opposition.players[4].startPOS = [opposition.players[4].originPOS[0], opposition.players[4].originPOS[1] + (matchHeight / 6)]
      ...
      opposition.players[5].startPOS = [opposition.players[5].originPOS[0], opposition.players[5].originPOS[1] + (matchHeight / 6)]
      opposition.players[6].startPOS = [opposition.players[6].originPOS[0], opposition.players[6].originPOS[1] + (matchHeight / 6)]
      opposition.players[7].startPOS = [opposition.players[7].originPOS[0], opposition.players[7].originPOS[1] + (matchHeight / 6)]
      opposition.players[8].startPOS = [opposition.players[8].originPOS[0], opposition.players[8].originPOS[1] + (matchHeight / 6)]
      ...
      team.players[9].startPOS = [team.players[9].originPOS[0], level - (matchHeight / 6)]
      team.players[10].startPOS = [team.players[10].originPOS[0], level - (matchHeight / 6)]
      opposition.players[1].startPOS = [opposition.players[1].originPOS[0], opposition.players[1].originPOS[1] + (matchHeight / 7)]
      opposition.players[2].startPOS = [opposition.players[2].originPOS[0], opposition.players[2].originPOS[1] + (matchHeight / 7)]
      opposition.players[3].startPOS = [opposition.players[3].originPOS[0], opposition.players[3].originPOS[1] + (matchHeight / 7)]
      opposition.players[4].startPOS = [opposition.players[4].originPOS[0], opposition.players[4].originPOS[1] + (matchHeight / 7)]
      opposition.players[5].startPOS = [opposition.players[5].originPOS[0], opposition.players[5].originPOS[1] + (matchHeight / 6)]
      opposition.players[6].startPOS = [opposition.players[6].originPOS[0], opposition.players[6].originPOS[1] + (matchHeight / 6)]
      opposition.players[7].startPOS = [opposition.players[7].originPOS[0], opposition.players[7].originPOS[1] + (matchHeight / 6)]
      opposition.players[8].startPOS = [opposition.players[8].originPOS[0], opposition.players[8].originPOS[1] + (matchHeight / 6)]
      opposition.players[9].startPOS = [opposition.players[9].originPOS[0], opposition.players[9].originPOS[1] + (matchHeight / 6)]
      opposition.players[10].startPOS = [opposition.players[10].originPOS[0], opposition.players[10].originPOS[1] + (matchHeight / 6)]
      ...
      team.players[1].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(0, (matchHeight / 6) - 5)]
      team.players[4].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(0, (matchHeight / 6) - 5)]
      team.players[6].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(0, (matchHeight / 6) - 5)]
      team.players[7].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(0, (matchHeight / 6) - 5)]
      team.players[8].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(0, (matchHeight / 6) - 5)]
      team.players[9].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(0, (matchHeight / 6) - 5)]
      team.players[10].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(0, (matchHeight / 6) - 5)]
      ...
      if (ballPosition[0] > matchWidth / 2) {
        ...
        opposition.players[8].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(0, (matchHeight / 6) - 5)]
        opposition.players[9].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(0, (matchHeight / 6) - 5)]
        ...
        opposition.players[5].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) - 5)), common.getRandomNumber(0, (matchHeight / 6) - 5)]
        opposition.players[9].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) - 5)), common.getRandomNumber(0, (matchHeight / 6) - 5)]
        opposition.players[10].startPOS = [matchWidth / 2, 20]
      } else {
        matchDetails.ball.direction = 'north'
    ...
    if (ballPosition[1] < (matchHeight / 3)) {
      ...
      matchDetails.ball.ballOverIterations = []
      team.players[0].startPOS = tempArray.map(x => x)
      team.players[1].startPOS = [team.players[1].originPOS[0], ((matchHeight / 6) + team.players[0].startPOS[1] + team.players[1].originPOS[1])]
      team.players[2].startPOS = [team.players[2].originPOS[0], ((matchHeight / 6) + team.players[0].startPOS[1] + team.players[2].originPOS[1])]
      team.players[3].startPOS = [team.players[3].originPOS[0], ((matchHeight / 6) + team.players[0].startPOS[1] + team.players[3].originPOS[1])]
      team.players[4].startPOS = [team.players[4].originPOS[0], ((matchHeight / 6) + team.players[0].startPOS[1] + team.players[4].originPOS[1])]
      team.players[5].startPOS = [team.players[5].originPOS[0], ((matchHeight / 6) + team.players[0].startPOS[1] + team.players[5].originPOS[1])]
      team.players[6].startPOS = [team.players[6].originPOS[0], ((matchHeight / 6) + team.players[0].startPOS[1] + team.players[6].originPOS[1])]
      team.players[7].startPOS = [team.players[7].originPOS[0], ((matchHeight / 6) + team.players[0].startPOS[1] + team.players[7].originPOS[1])]
      team.players[8].startPOS = [team.players[8].originPOS[0], ((matchHeight / 6) + team.players[0].startPOS[1] + team.players[8].originPOS[1])]
      team.players[9].startPOS = [team.players[9].originPOS[0], ((matchHeight / 6) + team.players[0].startPOS[1] + team.players[9].originPOS[1])]
      team.players[10].startPOS = [team.players[10].originPOS[0], ((matchHeight / 6) + team.players[0].startPOS[1] + team.players[10].originPOS[1])]
      opposition.players[1].startPOS = [opposition.players[1].originPOS[0], opposition.players[1].originPOS[1] - (matchHeight / 6)]
      opposition.players[2].startPOS = [opposition.players[2].originPOS[0], opposition.players[2].originPOS[1] - (matchHeight / 6)]
      opposition.players[3].startPOS = [opposition.players[3].originPOS[0], opposition.players[3].originPOS[1] - (matchHeight / 6)]
      opposition.players[4].startPOS = [opposition.players[4].originPOS[0], opposition.players[4].originPOS[1] - (matchHeight / 6)]
      ...
      //between halfway and last sixth
      const level = Math.round(common.getRandomNumber(ballPosition[1] + 15, (matchHeight - matchHeight / 9)))
      team.players[0].startPOS = [team.players[0].originPOS[0], team.players[5].startPOS[1] - (matchHeight / 3)]
      team.players[1].startPOS = [team.players[1].originPOS[0], team.players[5].startPOS[1] - (matchHeight / 6)]
      team.players[2].startPOS = [team.players[2].originPOS[0], team.players[5].startPOS[1] - (matchHeight / 6)]
      team.players[3].startPOS = [team.players[3].originPOS[0], team.players[5].startPOS[1] - (matchHeight / 6)]
      ...
      team.players[2].startPOS = [team.players[2].originPOS[0], team.players[2].originPOS[1] + (matchHeight / 2)]
      team.players[3].startPOS = [team.players[3].originPOS[0], team.players[3].originPOS[1] + (matchHeight / 2)]
      team.players[1].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(matchHeight - (matchHeight / 6) + 5, matchHeight)]
      team.players[4].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(matchHeight - (matchHeight / 6) + 5, matchHeight)]
      team.players[6].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(matchHeight - (matchHeight / 6) + 5, matchHeight)]
      team.players[7].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(matchHeight - (matchHeight / 6) + 5, matchHeight)]
      team.players[8].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(matchHeight - (matchHeight / 6) + 5, matchHeight)]
      team.players[9].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(matchHeight - (matchHeight / 6) + 5, matchHeight)]
      team.players[10].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(matchHeight - (matchHeight / 6) + 5, matchHeight)]
      ...
      if (ballPosition[0] > matchWidth / 2) {
        ...
        opposition.players[8].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(matchHeight - (matchHeight / 6) + 5, matchHeight)]
        opposition.players[9].startPOS = [common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5)), common.getRandomNumber(matchHeight - (matchHeight / 6) + 5, matchHeight)]
        ...
        opposition.players[5].startPOS[0] = common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5))
        opposition.players[5].startPOS[1] = common.getRandomNumber(matchHeight - (matchHeight / 6) + 5, matchHeight)
        opposition.players[9].startPOS[0] = common.getRandomNumber((matchWidth / 4) - 5, (matchWidth - (matchWidth / 4) + 5))
        opposition.players[9].startPOS[1] = common.getRandomNumber(matchHeight - (matchHeight / 6) + 5, matchHeight)
        opposition.players[10].startPOS = [matchWidth / 2, matchHeight + 20]
      } else {
        matchDetails.ball.direction = 'south'
        opposition.players[5].startPOS = [(matchWidth / 2) - 4, tempArray[1] + 40]
...
async function setGoalScored(scoringTeam, conceedingTeam, matchDetails) {
  const [matchWidth, matchHeight] = matchDetails.pitchSize
  await setVariables.resetPlayerPositions(scoringTeam)
    .catch(function(err) {
      const error = `Error when resetting player positions: ${err}`
      throw error
    })
  await setVariables.resetPlayerPositions(conceedingTeam)
    .catch(function(err) {
      const error = `Error when resetting player positions: ${err}`
      throw error
    })
...
async function keepInBoundaries(ballIntended, kickersSide, matchDetails) {
  ...
  if (ballIntended[0] < 0 || ballIntended[0] > matchWidth || ballIntended[1] < 0 || ballIntended[1] > matchHeight) {
    if (ballIntended[0] < 0) {
      if (kickersSide > matchHeight / 2) {
        let newBallIntended = await setThrowIn(kickOffTeam, secondTeam, 'left', ballIntended[1], matchDetails)
          .catch(function(err) {
            const error = `Error when setting up for a throwin: ${err}`
            throw error
          })
        matchDetails.iterationLog.push(`Throw in to - ${kickOffTeam.name}`)
        return newBallIntended
      }
      let newBallIntended = await setThrowIn(secondTeam, kickOffTeam, 'left', ballIntended[1], matchDetails)
        .catch(function(err) {
          const error = `Error when setting up for a throwin: ${err}`
          throw error
        })
      ...
      if (kickersSide > matchHeight / 2) {
        let newBallIntended = await setThrowIn(kickOffTeam, secondTeam, 'right', ballIntended[1], matchDetails)
          .catch(function(err) {
            const error = `Error when setting up for a throwin: ${err}`
            throw error
          })
      ...
      let newBallIntended = await setThrowIn(secondTeam, kickOffTeam, 'right', ballIntended[1], matchDetails)
        .catch(function(err) {
          const error = `Error when setting up for a throwin: ${err}`
          throw error
        })
    ...
    if (ballIntended[1] < 0) {
      ...
      if (kickersSide > matchHeight / 2) {
        let newBallIntended = await setGoalKick(kickOffTeam, secondTeam, matchDetails)
          .catch(function(err) {
            const error = `Error when setting the goal kick: ${err}`
            throw error
          })
      ...
      let newBallIntended = await setGoalKick(secondTeam, kickOffTeam, matchDetails)
        .catch(function(err) {
          const error = `Error when setting the goal kick: ${err}`
          throw error
        })
    ...
    if (teamAPlayer && teamAPlayer[0] === ballIntended[0] && teamAPlayer[1] === ballIntended[1]) {
      ...
      if (matchDetails.half === 1) {
        await setGoalScored(kickOffTeam, secondTeam, matchDetails)
          .catch(function(err) {
      ...
      await setGoalScored(secondTeam, kickOffTeam, matchDetails)
        .catch(function(err) {
          const error = `Error when processing the goal: ${err}`
          throw error
        })
...
async function setPlayerPositions(matchDetails, team, extra) {
  for (const thisPlayer of team.players) {
    ...
    if (playerPos <= matchDetails.pitchSize[1] || playerPos >= 0) {
...
async function formationCheck(origin, current) {
  try {
...
async function switchSide(team, matchDetails) {
  if (!team) {
    const error = 'No Team supplied to switch side'
    throw error
  }
  for (const thisPlayer of team.players) {
    if (!thisPlayer.originPOS) {
      const error = 'Each player must have an origin position set'
      throw error
    }
...
async function setRelativePosition(player, team, matchDetails) {
