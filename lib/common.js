...
function inTopPenalty(matchDetails, item) {
  ...
  let ballInPenalyBoxX = isBetween(item[0], (matchWidth / 4) + 6, matchWidth - (matchWidth / 4) - 6)
  let ballInTopPenalyBoxY = isBetween(item[1], 0, (matchHeight / 6) - 6)
...
function inBottomPenalty(matchDetails, item) {
  const [matchWidth, matchHeight] = matchDetails.pitchSize
  let ballInPenalyBoxX = isBetween(item[0], (matchWidth / 4) + 6, matchWidth - (matchWidth / 4) - 6)
  let ballInBottomPenalyBoxY = isBetween(item[1], matchHeight - (matchHeight / 6) + 6, matchHeight)
